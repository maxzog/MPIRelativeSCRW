module simulation_class
    ...
    type :: part
        ...
        real(8) :: taupi      !> Inverse particle response time
        real(8) :: pos(3)     !> Particle position
        real(8) :: fld(3)     !> Modeled velocity
        real(8) :: vel(3)     !> Particle velocity
    end type part
...
contains
   ...
   subroutine init(self, npart, length, numbins, delta, t, tf, dt, step, stepf, rank, numproc, taup) 
      implicit none
      class(simulation), intent(out) :: self
      integer, intent(in) :: npart, numbins, stepf, step, rank, numproc
      real(8), intent(in) :: length, tf, dt, delta, t, taup
      integer :: i
   ...
   subroutine write_particle_data(this, filename)
      ...
      call MPI_FILE_WRITE_AT_ALL(io, disp, this%ps, this%npart*10, MPI_DOUBLE_PRECISION, status, ierr)
  ...
  subroutine compute_w1(this)
   ...
   do i=1,this%npart
      ...
      this%w1(ir) = this%w1(ir) + dot_product(this%ps(i)%fld,rhat) 
  ...
  subroutine compute_w2(this)
   ...
   do i=1,this%npart
      ...
      this%w2(ir) = this%w2(ir) + (dot_product(this%ps(i)%fld,rhat) - this%w1(ir))**2
      this%c(ir) = this%c(ir) + 1
   end do
